...
def list_videos():
    try:
        resp = s3.list_objects_v2(Bucket=HLS_BUCKET, Prefix="hls/", Delimiter="/")
    except ClientError:
...
def stream_file(video_id: str, filename: str):
    ...
    content_type = "application/vnd.apple.mpegurl" if is_playlist else "video/MP2T"
...
def player(request: Request, video_id: str):
    ...
    return templates.TemplateResponse(
        request=request,
        name="player.html",
        context={"video_id": video_id, "stream_url": stream_url},
    )
